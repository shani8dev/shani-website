...
- **CI status — corrected, was stale.** `.github/workflows/ci.yml` runs on every push/PR touching `index.html`: html5lib strict parse (0 parser errors), SRI verification on every `integrity=` CDN resource, sitemap.xml well-formedness, and a version-lockstep check that requires every release string in `index.html` to match `assets/js/script.js`'s `VERSION` constant (roadmap #4). Plus a shared secrets scan job. Adopted `shani-ci-commons` templates 2026-09-20.
...
2. **No test infrastructure** — `garuda-ng` has Vitest unit tests + Playwright e2e; shani-website has no unit/e2e test suite of its own, but it does have CI now (`.github/workflows/ci.yml`, 2026-09-20: html5lib parse + SRI + sitemap + version lockstep + secrets scan).
